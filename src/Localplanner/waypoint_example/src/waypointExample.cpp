...
int main(int argc, char **argv)
{
    ...
    ros::Publisher pubWaypoint = nh.advertise<geometry_msgs::PoseStamped>("/way_point", 5);
    ...
    while (status)
    {
        ...
        if (curTime - waypointTime > 1.0 / frameRate)
        {
            if (!isWaiting)
            {
                ...
                waypointMsgs.pose.position.x = waypoints->points[wayPointID].x;
                waypointMsgs.pose.position.y = waypoints->points[wayPointID].y;
                waypointMsgs.pose.position.z = waypoints->points[wayPointID].z;
