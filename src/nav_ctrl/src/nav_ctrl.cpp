...
void teleop_ctrl(const geometry_msgs::TwistConstPtr &msg)
{
    /** 控制状态标志位
      enum ctrl_status_t{
        CTRL_RELEASED = 0,
     ...
     * in_control()返回控制状态，处于CTRL_OBTAINED才能控制
        1.初始化状态时状态为CTRL_RELEASED
        2.调用SerialDisconnectHandle()将状态置为CTRL_RELEASED
        3.release_control()将状态置为CTRL_RELEASED
     */
    if (!pMovementCtrl->in_control()) // 未获取控制权直接退出（这应该是在检测遥控是否有开启）
    {
    ...
    {
        pMovementCtrl->ctrl_data.forward = msg->linear.x > 0 ? 1.5 : -1.5;
    }
    else
    {
    ...
    {
        pMovementCtrl->ctrl_data.left = msg->angular.z > 0 ? 2.4 : -2.4;
    }
    else
    {
    ...
    pMovementCtrl->ctrl_data.roll = 0;
    pMovementCtrl->ctrl_data.pitch = 0;
    pMovementCtrl->ctrl_data.up = 0;
...
int main(int argc, char **argv)
{
    ...
    init_control_mode();
    ros::Subscriber sub = nh.subscribe<geometry_msgs::Twist>("/cmd_vel", 1, teleop_ctrl); // subscribe to ROS topic
    ros::spin();
